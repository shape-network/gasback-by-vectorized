...
contract Gasback {
	...
	function setAccuralWithdrawer(address addr, bool authorized) public onlySystemOrThis returns (bool) {
	...
	fallback() external payable {
		...
		assembly {
			if gasToBurn {
				...
				if lt(sub(gasBefore, gas()), gasToBurn) {
					revert(0x00, 0x00)
				}
			}
			...
			if ethToGive {
				...
				if iszero(call(gas(), caller(), ethToGive, 0x00, 0x00, 0x00, 0x00)) {
					...
					if iszero(create(ethToGive, 0x0b, 0x16)) {
						revert(0x00, 0x00)
					}
				}
			}
