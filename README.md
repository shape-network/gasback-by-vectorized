...
- The `baseFeeVault` recipient is set to `FeeVaultSplitter`.
- `Gasback` receives only its configured share from `FeeVaultSplitter`.

### Via script

See `script/DeployGasback.s.sol` and `script/DeployFeeVaultSplitter.s.sol` for deployment scripts.
...
2. Deploy `FeeVaultSplitter` with `Gasback` as one of the payees.

3. Set the `baseFeeVault` recipient to the deployed `FeeVaultSplitter`.
